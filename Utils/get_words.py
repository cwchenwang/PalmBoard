...
def get_sd(path, mode, name, sentences, f):
    ...
    for sentence in sentences:
        for char in sentence:
            char = char.lower()
            if(char == ' '):
                spacex.append(data[char][counter[char]].x)
                spacey.append(data[char][counter[char]].y)
            else:
                charx.append(data[char][counter[char]].x)
                chary.append(data[char][counter[char]].y)
            counter[char] += 1
        num_sent += 1
        if(num_sent % 15 == 0):
...
# f.write("subject,block,mode,speed,")
# for name in names:
...
for mode in ["hover", "place"]:
    tot_words = 0
    for name in names:
        tot_words += len(get_words("/Volumes/Seagate Exp/data/", mode, name))
    print(mode, tot_words)